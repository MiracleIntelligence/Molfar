...
namespace Molfar.Notes
{
    public class MolfarNotesProcessor : MolfarCommandProcessor
    {
        ...
        private Task<MolfarAnswer> SaveNote(string message)
        {
            ...
            return Task.FromResult(new MolfarAnswer($"SAVED: {arg2}"));
        }

        private Task<MolfarAnswer> GetNote(int v)
        {
            var note = _databaseService.Connection.Table<Note>().ElementAt(v);
            return Task.FromResult(new MolfarAnswer($"{note.Title} - {note.Text}"));
        }

        private Task<MolfarAnswer> GetAllNotes()
        {
            var count = _databaseService.Connection.Table<Note>().Count();
            return Task.FromResult(new MolfarAnswer($"COUNT: {count}"));
        }

        private Task<MolfarAnswer> GetRandomNote()
        {
            var count = _databaseService.Connection.Table<Note>().Count();
